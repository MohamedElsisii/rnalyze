...
handle_reference_genome() {
    ...
    case "$REF_METHOD" in
        "UnindexedURL")
            ...
            ref_ext="${ref_file_name##*.}"
            if [[ "$ref_ext" != "fa" && "$ref_ext" != "fna" && "$ref_ext" != "fasta" ]]; then
                log "ERROR" "Invalid reference genome extension: .$ref_ext. Expected .fa, .fna, or .fasta."
                exit 1
            fi

            # Set the reference genome name to 'ref_genome' while keeping its extension
            REF_GENOME_FILE="$LOCATION/Ref_Genome/ref_genome.$ref_ext"
            REF_GENOME_EXT="$ref_ext"
            log "INFO" "Reference genome file: $REF_GENOME_FILE"
            log "INFO" "Reference genome extension: .$REF_GENOME_EXT"
        ...
        "IndexedURL")
            ...
            ref_file_name=$(basename "$(ls "$LOCATION/Ref_Genome"/*.{fa,fna,fasta} 2>/dev/null | head -n 1)")
        ...
        "UnindexedPath")
            ...
            if [[ -d "$REF_PATH" ]]; then
                ...
                ref_files=($(find "$REF_PATH" -maxdepth 1 -type f \( -name "*.fa" -o -name "*.fna" -o -name "*.fasta" \)))
                ...
                if [[ ${#ref_files[@]} -eq 0 ]]; then
                    ...
                    log "WARNING" "Multiple reference genome files found. Using the first file: ${ref_files[0]}"
                fi
            ...
            cp "$ref_file" "$LOCATION/Ref_Genome/" || { log "ERROR" "Failed to copy reference genome file. Exiting..."; exit 1; }
        ...
        "IndexedPath")
            ...
            ref_file_name=$(basename "$(ls "$REF_PATH"/*.{fa,fna,fasta} 2>/dev/null | head -n 1)")
            if [[ -z "$ref_file_name" ]]; then
                log "ERROR" "No reference genome file found with .fa, .fna, or .fasta extension in $REF_PATH."
                exit 1
            fi
...
perform_alignment() {
    ...
    if [[ -f "$LOCATION/Ref_Genome/ref_genome.fna" ]]; then
    	REF_GENOME="$LOCATION/Ref_Genome/ref_genome.fna"
    elif [[ -f "$LOCATION/Ref_Genome/ref_genome.fa" ]]; then
    	REF_GENOME="$LOCATION/Ref_Genome/ref_genome.fa"
    elif [[ -f "$LOCATION/Ref_Genome/ref_genome.fasta" ]]; then
    	REF_GENOME="$LOCATION/Ref_Genome/ref_genome.fasta"
    else
    	log "ERROR" "No reference genome file found in $LOCATION/Ref_Genome with .fna, .fa, or .fasta extension."
    	exit 1
    fi
    ...
    if [[ "$DATA_TYPE" == "SE" ]]; then
        ...
        for file in "$LOCATION/Data"/*_${EXTENSION}.fastq.gz; do
            ...
            case $TOOL in
                "BWA")
                    bwa mem -t "$MAX_THREADS" "$REF_GENOME" "$file" > "$LOCATION/Mapping/${filename}.sam"
                    ;;
                "Bowtie2")
        ...
        for r1 in "$LOCATION/Data"/*_1_${EXTENSION}.fastq.gz; do
            ...
            if [[ "$r1" == *"_1_${EXTENSION}.fastq.gz" ]]; then
                ...
                case $TOOL in
                    "BWA")
                        bwa mem -t "$MAX_THREADS" "$REF_GENOME" "$r1" "$r2" > "$LOCATION/Mapping/${sample}.sam"
                        ;;
                    "Bowtie2")
    ...
    if [[ "$PIPELINE" == "Full" ]]; then
        ...
        case "${GTF_METHOD:-}" in
            ...
            "Path")
                ...
                cp "$gtf_file" "$LOCATION/Annotation/annotation.gtf" || { log "ERROR" "Failed to copy GTF file. Exiting..."; exit 1; }
                ;;
