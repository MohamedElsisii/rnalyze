...
handle_reference_genome() {
    ...
    case "$REF_METHOD" in
        "UnindexedURL")
            ...
            # Uncompress if the file is gzipped
            if [[ "$ref_file_name" == *.gz ]]; then
                ...
                ref_file_name="${ref_file_name%.gz}"  # Remove .gz extension
            fi
            ...
            # Store the reference genome file name and extension for later use
        ...
        "IndexedURL")
            ...
            # Detect the reference genome file (e.g., ref_genome.fa, ref_genome.fna, ref_genome.fasta)
        ...
        "UnindexedPath")
            ...
            ref_file_name=$(basename "$REF_PATH")
            ref_ext="${ref_file_name##*.}"
            if [[ "$ref_ext" != "fa" && "$ref_ext" != "fna" && "$ref_ext" != "fasta" ]]; then
                log "ERROR" "Invalid reference genome extension: .$ref_ext. Expected .fa, .fna, or .fasta."
                exit 1
            ...
            # Copy the file to the Ref_Genome directory
            cp "$REF_PATH" "$LOCATION/Ref_Genome/$ref_file_name" || { log "ERROR" "Failed to move file. Exiting..."; exit 1; }
        ...
        "IndexedPath")
            ...
            # Store the reference genome file name and extension for later use
            REF_GENOME_FILE="$REF_PATH/$ref_file_name"
            REF_GENOME_EXT="$ref_ext"
            log "INFO" "Reference genome file: $REF_GENOME_FILE"
            log "INFO" "Reference genome extension: .$REF_GENOME_EXT"
